...
class Controller {
    ...
    on_receive_updates(ob: {updates: string | any[];}) {
    ...
    onClick = (event: MouseEvent) => {
        ...
        const response = fetch('ajax.html', {
            method: "POST",
            body: JSON.stringify(payload)
    ...
    update() {
        ...
        const place_bomb = async(time1: number, time2: number) => {

            return new Promise(async (resolve) => {

                let bomb = new Sprite("bomb",this.model.sprites[0].x, this.model.sprites[0].y, "bomb.png", Sprite.prototype.sit_still, Sprite.prototype.ignore_click);
                ...
                bomb.image.src = "explosion.png"

                let bombIndex = this.model.sprites.indexOf(bomb)
    
                await sleep(time2)

                this.model.sprites.splice(bombIndex, 1)
